...
COLOR_RULES = [
    ...
    ("(?<![黄青苔金銀灰])緑(?!青)", lambda h, s, l: s < 12 or 60 <= h <= 195, "緑の範囲外"),
    # 日本語の「青」は緑寄りまで含む(青葉・青信号)。藍や紺より広く取る。
    ("藍|紺|瑠璃",          lambda h, s, l: s < 12 or 195 <= h <= 265,     "青の範囲外"),
    ("(?<!緑)青(?![緑磁竹海])", lambda h, s, l: s < 12 or 170 <= h <= 270,   "青の範囲外"),
    ("朱|緋|紅|赤(?![茶褐紫])", lambda h, s, l: s < 12 or h >= 330 or h <= 30, "赤の範囲外"),
    ("紫(?!蘇)",           lambda h, s, l: s < 12 or 228 <= h <= 335,      "紫の範囲外"),
    # 「白茶」「赤褐」のように茶が芯になる複合名が多い。茶がないと
    # 前half の「白」で判定してしまう。
    ("茶(?!碗|室|器|道)|褐",  lambda h, s, l: s < 12 or h <= 55 or h >= 350,  "茶の範囲外"),
]
