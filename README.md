...
[js/dictionary.js](js/dictionary.js) の配列にエントリを足す。

```js
{ match: ["onsen", "温泉", "湯けむり"],   // 受理する言葉(英語・日本語いくつでも)
  ja: "温泉", story: "湯けむりと檜の香り",  // 資料に出る物語の一文
  anchors: [                               // 呼び起こす色(2〜4色)
    { h: 40, s: 15, l: 88, name: "乳白色の湯" },
    { h: 25, s: 40, l: 45, name: "檜の湯船" },
  ],
  toneBias: ["sf", "ltg"],  // トーンの気配: p優しい/lt爽やか/b陽気/v鮮やか/sf穏やか/
                            //   d くすみ/dp深い和/dk大人/g灰みシック/dkg重厚/ltg淡く落ち着く
  matte: true,              // ツヤケシ優遇(キラキラ系は sparkle: true)
  // technique: "対照トーン配色" など指定も可(未指定なら自動判定)
}
```

h(色相)早見: 0赤 / 30橙 / 50黄 / 90黄緑 / 140緑 / 180ターコイズ / 220青 / 260青紫 / 290紫 / 330ピンク。
保存後 `sh tools/deploy-lolipop.sh` で本番反映。
